...
/**
 * @file
 * Contains \Drupal\blog_publish\Form\EditUpdateForm.
 */
...
class EditUpdateForm extends FormBase {
  ...
  public function getFormId() {
    return 'blog_publish_edit_update_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $entity = NULL, $field_delta = NULL) {
    ...
    $form_state->set(['blog_publish', 'entity'], $entity);
    $form_state->set(['blog_publish', 'field'], $field);
    $form_state->set(['blog_publish', 'field_delta'], $field_delta);
    ...
    $form['value'] = [
      ...
      '#description' => $this->t('Date & time of the blog scheduled state change'),
      '#date_increment' => 1,
      '#date_timezone' => date_default_timezone_get(),
      '#default_value' => new DrupalDateTime($states[$field_delta]['value'], BlogPublish::STORAGE_TIMEZONE),
      '#required' => TRUE,
    ];
    ...
    $url = Url::fromRoute('view.blog_publish.page_1');
  ...
  public function handleStates(FormStateInterface $form_state, &$states) {
    $entity = $form_state->get(['blog_publish', 'entity']);
    $orig_status = $entity->moderation_state->value;
    $m_options = $this->getModerationOptions($entity);
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $value = $date->format(BlogPublish::DATETIME_STORAGE_FORMAT);

    $entity = $form_state->get(['blog_publish', 'entity']);
    $field = $form_state->get(['blog_publish', 'field']);
    $delta = $form_state->get(['blog_publish', 'field_delta']);
    $states = $entity->get($field)->getValue();
