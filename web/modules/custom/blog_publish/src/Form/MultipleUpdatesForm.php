...
/**
 * @file
 * Contains \Drupal\blog_publish\Form\MultipleUpdatesForm.
 */

namespace Drupal\blog_publish\Form;

use Drupal\Core\Datetime\DrupalDateTime;
use Drupal\Core\Form\FormBase;
use Drupal\Core\Form\FormStateInterface;
use Drupal\workflows\Entity\Workflow;
use Drupal\blog_publish\Plugin\Field\FieldType\BlogPublish;
...
class MultipleUpdatesForm extends FormBase {
  ...
  public function getFormId() {
    return 'blog_publish_multiple_updates_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $wrapper = 'blog-publish-form-wrapper';
    $form['#tree'] = TRUE;
    $form['#prefix'] = '<div id="' . $wrapper . '">';
    $form['#suffix'] = '</div>';

    // CONTENT MODERATION WORKFLOWS
    $workflow_objects = $form_state->get(['blog_publish', 'workflow_objects']);
    if (!$workflow_objects) {
      $workflow_objects = Workflow::loadMultipleByType('content_moderation');
      $form_state->set(['blog_publish', 'workflow_objects'], $workflow_objects);
    }
    if (!$workflow_objects) {
      $form['message'] = [
    ...
    $form['warning'] = [
      '#theme_wrappers' => ['container'],
      '#markup' => $this->t('Upon saving all existing blog publishing entries for the selected entities will be removed and new ones added.<br>The first entity is used to determine valid state changes. Invalid options for any following entities will not be saved.'),
    ];
    ...
    $workflow = $form_state->get(['blog_publish', 'workflow']);
    if (!$workflow) {
      $workflow = key($workflows);
      $form_state->set(['blog_publish', 'workflow'], $workflow);
    }
    ...
    $entities = $form_state->get(['blog_publish', 'entities']);
    $entities_count = 0;
    if (!isset($entities)) {
      $entities = [];
      $form_state->set(['blog_publish', 'entities'], $entities);
    }
    ...
    $states = $form_state->get(['blog_publish', 'states']);
    if (!isset($states)) {
      $states = [];
      $form_state->set(['blog_publish', 'states'], $states);
    }
    elseif (count($states)) {
    ...
    $entity = $form_state->get(['blog_publish', 'first_entity']);
    if (!$entity) {
      $entity = \Drupal::entityTypeManager()->getStorage('node')->load(reset($entities));
      $form_state->set(['blog_publish', 'first_entity'], $entity);
    }
    ...
    $form['states']['value'] = [
      ...
      '#description' => $this->t('The datetime of the blog publish'),
      '#date_increment' => 1,
  ...
  protected function buildStateEditForm(&$form, $wrapper, $entry) {
    ...
    $form['value'] = [
      '#type' => 'datetime',
      '#title' => $this->t('Scheduled date'),
      '#default_value' => new DrupalDateTime($entry['date'], BlogPublish::STORAGE_TIMEZONE),
      '#date_increment' => 1,
      '#date_timezone' => date_default_timezone_get(),
      '#element_validate' => [],
    ];
  ...
  public function handleSubmissions(FormStateInterface $form_state) {
    ...
    if ($triggering_element['#name'] === $button_name) {
      ...
      if ($values['value']['date'] && $values['value']['time']) {
        ...
        $date->setTimezone(new \DateTimezone(BlogPublish::STORAGE_TIMEZONE));
        $states[$triggering_element['#sp_row_delta']] = [
          'date' => $date->format(BlogPublish::DATETIME_STORAGE_FORMAT),
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $message = [
      '#theme' => 'item_list',
      '#list_type' => 'ul',
      '#title' => $this->t('Added blog publishing entries for the following entities:'),
