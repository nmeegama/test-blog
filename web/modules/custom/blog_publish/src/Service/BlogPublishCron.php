...
namespace Drupal\blog_publish\Service;
...
/**
 * Class BlogPublishCron
 *
 * @package Drupal\blog_publish\Service
 */
class BlogPublishCron {
  ...
  private function updateEntityField(ContentEntityBase $entity, string $scheduledField): void {
    ...
    foreach ($scheduledValue as $key => $value) {
      $test1= $this->getTimestampFromIso8601($value['value']);
      $test2 = $this->dateTime->getCurrentTime();
  ...
  private function getTimestampFromIso8601(string $dateIso8601): int {
    $datetime = new DateTime($dateIso8601, new DateTimeZone(BlogPublish::STORAGE_TIMEZONE));
