...
namespace Drupal\blog_publish\Plugin\Field\FieldWidget;
...
/**
 * Plugin implementation of the 'blog_publish_widget' widget.
 *
 * @FieldWidget(
 *   id = "blog_publish",
 *   label = @Translation("Blog Publish"),
 *   field_types = {
 *     "blog_publish"
 *   },
 ...
 * )
 */
class BlogPublishWidget extends WidgetBase {

  /**
   * {@inheritdoc}
   */
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state): array {
    $element['#attached']['library'][] = 'blog_publish/blog_publish_widget';
    ...
    $wrapper = 'blog-publish-form-' . $field_name;
    $element = [
      '#type' => 'details',
      '#title' => t('Blog moderation state change'),
      ...
      '#tree' => TRUE,
      '#sp_root' => TRUE,
      '#prefix' => '<div id="' . $wrapper . '" class="blog-publish-wrapper">',
      '#suffix' => '</div>',
    ...
    $entries = $form_state->get(['blog_publish', $field_name]);
  ...
  protected function prepareFormState(FormStateInterface $form_state, FieldItemListInterface $items, $field_name) {
    ...
    if (!isset($entries)) {
      ...
      $form_state->set(['blog_publish', $field_name], $entries);
  ...
  protected function addEntries(&$element, FormStateInterface $form_state, $parents, $field_name, $wrapper) {
    ...
    $element['entries'] = [
      '#type' => 'container',
      '#attributes' => ['class' => 'blog-publish-entries'],
    ];
    foreach ($entries as $key => $value) {
      ...
      if (!empty($value['form'])) {
        ...
        $element['entries'][$key]['form'] = [
          '#type' => 'container',
          '#attributes' => ['class' => 'blog-publish-entry-form'],
        ...
        $element['entries'][$key] = [
          '#type' => 'container',
          '#attributes' => ['class' => 'blog-publish-entry'],
          'date' => [
            '#type' => 'html_tag',
            '#tag' => 'div',
          ...
          'state' => [
            ...
            '#attributes' => ['class' => 'blog-publish-entry-state'],
          ],
          'actions' => [
            '#type' => 'container',
            '#attributes' => ['class' => 'blog-publish-entry-actions'],
            'edit' => [
              '#type' => 'button',
              '#value' => $this->t('Edit'),
  ...
  public function handleSubmissions(FormStateInterface $form_state) {
    ...
    $entries = $form_state->get(['blog_publish', $triggering_element['#sp_id']]);
    ...
    $form_state->set(['blog_publish', $triggering_element['#sp_id']], $entries);
