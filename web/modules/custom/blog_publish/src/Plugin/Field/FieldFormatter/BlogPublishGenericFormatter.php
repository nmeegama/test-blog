...
namespace Drupal\blog_publish\Plugin\Field\FieldFormatter;
...
/**
 * Plugin implementation of the 'blog_publish_generic_formatter'
 * formatter.
 *
 * @FieldFormatter(
 *   id = "blog_publish_generic_formatter",
 *   label = @Translation("Blog publish generic formatter"),
 *   field_types = {
 *     "blog_publish"
 *   }
 * )
 */
class BlogPublishGenericFormatter extends FormatterBase implements ContainerFactoryPluginInterface {
  ...
  /**
   * BlogPublishGenericFormatter constructor.
   *
   * @param $plugin_id
  ...
  public function __construct($plugin_id, $plugin_definition, FieldDefinitionInterface $field_definition, array $settings, $label, $view_mode, array $third_party_settings, LoggerChannelFactoryInterface $loggerChannelFactory, EntityTypeManagerInterface $entityTypeManager) {
    ...
    $this->logger = $loggerChannelFactory->get('blog_publish');
  ...
  public function viewElements(FieldItemListInterface $items, $langcode): array {
    ...
    foreach ($items as $delta => $item) {
      /**
       * @var $item \Drupal\blog_publish\Plugin\Field\FieldType\BlogPublish
       */
  ...
  private function parseDate(string $strDateTime, string $strDateFormat): string {
    ...
    if ($dateFormat !== NULL) {
      ...
      $drupalDateTime = DrupalDateTime::createFromFormat(BlogPublish::DATETIME_STORAGE_FORMAT, $strDateTime, BlogPublish::STORAGE_TIMEZONE);
